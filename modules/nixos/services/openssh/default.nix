...
let
  ...
  other-hosts = lib.filterAttrs (key: host:
    key != name && (host.config.caramelmint.user.name or null) != null)
  ...
  other-hosts-config = lib.concatMapStringsSep "\n" (name:
    let
      ...
      remote-user-name = remote.config.caramelmint.user.name;
      remote-user-id =
...
in {
  options.caramelmint.services.openssh = with types; {
  ...
  config = mkIf cfg.enable {
    ...
    caramelmint.user.extraOptions.openssh.authorizedKeys.keys =
      cfg.authorizedKeys;

    caramelmint.home.extraOptions = {
      programs.fish.shellAliases = foldl (aliases: system:
        aliases // {
